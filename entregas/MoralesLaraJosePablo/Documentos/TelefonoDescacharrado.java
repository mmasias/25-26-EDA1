...
public class TelefonoDescacharrado {
    ...
    private List<Nino> niñosJugando;
    private List<Nino> ninosConLydia;
    ...
    public TelefonoDescacharrado() {
        ...
        this.niñosJugando = new ArrayList<Nino>();
        this.ninosConLydia = new ArrayList<Nino>();
    ...
    private void simularLlegadaNinos() {
        ...
        for (int i = 0; i < niñosLlegan; i++) {
            Nino nuevoNiño = new Nino("Nino" + (getTotalNinos() + 1));
            
            if (juegoEnCurso) {
                ninosConLydia.add(nuevoNiño);
                System.out.println("Minuto " + tiempoActual + ": " + nuevoNiño.getNombre() + 
                                 " llega y se queda con Lydia (juego en curso)");
            } else {
                colaEspera.offer(nuevoNiño);
                System.out.println("Minuto " + tiempoActual + ": " + nuevoNiño.getNombre() + 
    ...
    private void iniciarJuego() {
        ...
        niñosJugando.clear();
        int niñosAJugar = Math.min(colaEspera.size(), 5 + random.nextInt(4));
        
        for (int i = 0; i < niñosAJugar && !colaEspera.isEmpty(); i++) {
            niñosJugando.add(colaEspera.poll());
        }
        
        System.out.println("\nAisha limpia la pizarra del salon");
        for (int i = 0; i < niñosJugando.size(); i++) {
            Nino nino = niñosJugando.get(i);
        ...
        System.out.println("Ninos participantes: " + niñosJugando.size());
        ...
        ResultadoJuego resultado = new ResultadoJuego(numeroJuego, mensajeOriginal, 
                                                    mensajeFinal, niñosJugando.size(), 
                                                    tiempoActual);
        resultadosJuegos.add(resultado);
        
        int tiempoJuego = niñosJugando.size() + 1;
    ...
    private String simularPasoMensaje(String mensajeOriginal) {
        ...
        for (int i = 0; i < niñosJugando.size(); i++) {
            Niño niño = niñosJugando.get(i);
            
            mensajeActual = deformarMensaje(mensajeActual);
            
            if (i == 0) {
                System.out.println("  " + niño.getNombre() + " (primero) recibe: \"" + mensajeActual + "\"");
            } else if (i == niñosJugando.size() - 1) {
                System.out.println("  " + niño.getNombre() + " (ultimo) escribe en pizarra: \"" + mensajeActual + "\"");
            } else {
                System.out.println("  " + niño.getNombre() + " pasa: \"" + mensajeActual + "\"");
            }
        }
    ...
    private void finalizarJuego() {
        ...
        for (Nino niño : niñosConLydia) {
            colaEspera.offer(niño);
            System.out.println("  " + nino.getNombre() + " pasa de Lydia a la cola");
        }
        ninosConLydia.clear();
        
        for (Nino niño : niñosJugando) {
            colaEspera.offer(niño);
        }
        
        System.out.println("Cola actual: " + colaEspera.size() + " ninos\n");
    }
    ...
    private void mostrarEstadisticasFinales() {
        ...
        System.out.println("Total de niños que participaron: " + getTotalNiños());
